#!/usr/bin/env python3
"""
HYBRID APPROACH: LinearRegression + StackingRegressor with Multiple Ridge Configs
===============================================================================
Strategy: Test simple LinearRegression vs proven StackingRegressor with 
different Ridge alpha combinations to find optimal ensemble configuration.

Goal: Generate multiple CSV submissions to test various approaches
"""
...
print("🔄 HYBRID APPROACH: LinearRegression + Multiple Ridge Stacking")
print("=" * 70)
print("Testing: Basic LinearRegression vs StackingRegressor variations")
print("Output: Multiple CSV files for systematic testing")
print()
...
def generate_multiple_submissions(X, y, X_test, test_data):
    """Generate multiple CSV submissions with different approaches"""
    ...
    # Pure LinearRegression
    lr_basic = LinearRegression()
    cv_scores = cross_val_score(lr_basic, X, y, cv=5, scoring='neg_mean_absolute_error', n_jobs=-1)
    cv_mae = -cv_scores.mean()
    cv_std = cv_scores.std()
    expected_kaggle = cv_mae + 0.28
    
    print(f"📊 Basic LinearRegression:")
    print(f"   CV MAE: {cv_mae:.5f} ± {cv_std:.5f}")
    print(f"   Expected Kaggle: {expected_kaggle:.5f}")
    
    # Generate submission
    lr_basic.fit(X, y)
    predictions = lr_basic.predict(X_test)
    predictions = np.clip(predictions, 0, 120)
    predictions = np.round(predictions).astype(int)
    
    submission_file = f"csv/submission_LinearRegression_basic_{timestamp}.csv"
    submission = pd.DataFrame({'ID': test_data['ID'], 'W': predictions})
    submission.to_csv(submission_file, index=False)
    submissions.append(('LinearRegression_Basic', submission_file, expected_kaggle))
    
    print(f"   📁 Saved: {submission_file}")
    print()
    
    print("🔍 2. STACKING WITH DIFFERENT RIDGE CONFIGURATIONS")
    print("=" * 50)
    
    # Different Ridge configurations to test
    ridge_configs = [
        # Very conservative (high alpha)
        {
            'name': 'VeryConservative',
            'alphas': [2.0, 8.0, 5.0],
            'meta_alpha': 4.0,
            'description': 'High regularization for maximum stability'
        },
        # Original proven config
        {
            ...
            'alphas': [1.0, 5.0, 2.0],
            'meta_alpha': 2.0,
            'description': 'Your proven configuration'
        },
        # Light regularization
        {
            'name': 'LightRegularization',
            'alphas': [0.5, 2.0, 1.0],
            'meta_alpha': 1.0,
            'description': 'Lower regularization for more flexibility'
        },
        # Diverse alphas
        {
            'name': 'DiverseAlphas',
            'alphas': [0.1, 10.0, 1.0],
            'meta_alpha': 2.0,
            'description': 'Wide range of regularization strengths'
        },
        # Balanced approach
        {
            'name': 'Balanced',
    ...
    for config in ridge_configs:
        ...
        cv_mae = -cv_scores.mean()
        cv_std = cv_scores.std()
        expected_kaggle = cv_mae + 0.30  # Conservative gap estimate
        ...
        submission_file = f"csv/submission_Stacking_{config['name']}_{timestamp}.csv"
        submission = pd.DataFrame({'ID': test_data['ID'], 'W': predictions})
        submission.to_csv(submission_file, index=False)
        submissions.append((f"Stacking_{config['name']}", submission_file, expected_kaggle))
        
        print(f"   📁 Saved: {submission_file}")
        print()
    
    print("🔍 3. HYBRID: LINEAR REGRESSION IN STACKING")
    print("=" * 50)
    
    # Hybrid: Mix LinearRegression with Ridge in stacking
    hybrid_configs = [
        {
            'name': 'LR_Ridge_Mix',
            'estimators': [
                ...
                ('ridge2', Ridge(alpha=3.0, random_state=42))
            ],
            'meta_alpha': 1.5,
            'description': 'LinearRegression + 2 Ridge models'
        },
        {
            'name': 'Dual_LR_Ridge',
            'estimators': [
                ('linear1', LinearRegression()),
                ('linear2', LinearRegression()),  # Same model, different CV folds
                ('ridge', Ridge(alpha=2.0, random_state=42))
            ],
            'meta_alpha': 1.0,
            'description': '2 LinearRegression + 1 Ridge'
        }
    ]
    
    for config in hybrid_configs:
        hybrid_stacking = StackingRegressor(
        ...
        cv_scores = cross_val_score(hybrid_stacking, X, y, cv=5, scoring='neg_mean_absolute_error', n_jobs=-1)
        cv_mae = -cv_scores.mean()
        cv_std = cv_scores.std()
        expected_kaggle = cv_mae + 0.29  # Hybrid might have slightly better gap
        
        print(f"📊 Hybrid {config['name']}:")
        print(f"   Config: {config['description']}")
        ...
        # Generate submission
        hybrid_stacking.fit(X, y)
        predictions = hybrid_stacking.predict(X_test)
        predictions = np.clip(predictions, 0, 120)
        predictions = np.round(predictions).astype(int)
        
        submission_file = f"csv/submission_Hybrid_{config['name']}_{timestamp}.csv"
        submission = pd.DataFrame({'ID': test_data['ID'], 'W': predictions})
        submission.to_csv(submission_file, index=False)
        submissions.append((f"Hybrid_{config['name']}", submission_file, expected_kaggle))
...
submissions = generate_multiple_submissions(X_train, y_train, X_test, test)

print("🏆 SUBMISSION SUMMARY")
print("=" * 60)
print("Generated submissions for systematic testing:")
print()

best_expected = float('inf')
best_name = None

for name, file_path, expected_mae in submissions:
    print(f"📁 {name}")
...
print(f"🥇 Best Expected Performance: {best_name}")
print(f"🎯 Expected MAE: {best_expected:.5f}")
print()
print("💡 Testing Strategy:")
print("   1. Submit each CSV to compare actual vs expected performance")
print("   2. Identify which Ridge configuration works best")
print("   3. Compare LinearRegression vs StackingRegressor approaches")
print("   4. Find optimal balance between simplicity and ensemble power")
print()
print("📊 Current benchmark to beat: 3.02469 MAE")